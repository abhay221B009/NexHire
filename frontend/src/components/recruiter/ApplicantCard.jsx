...
import { Mail, Phone, MapPin, Download, Briefcase, GraduationCap, CheckCircle2, ChevronDown } from 'lucide-react';
...
export default function ApplicantCard({ application, onStatusUpdated }) {
  ...
  const [currentStage, setCurrentStage] = useState(application.stage || 'Applied');
  ...
  const handleStageChange = async (newStage) => {
    ...
    setUpdating(true);
    try {
      const response = await api.patch(`/applications/${application._id}/status`, { stage: newStage });
      if (response.data?.success) {
        setCurrentStage(newStage);
        showSuccess(`Candidate moved to stage: ${newStage}`);
        if (onStatusUpdated) onStatusUpdated(application._id, newStage);
      }
    } catch (error) {
      showError(error.customMessage || 'Failed to update candidate application status');
    } finally {
      setUpdating(false);
    }
  };
    ...
    <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs flex flex-col gap-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2">
          <label className="text-xs font-bold text-slate-400 hidden sm:inline">Stage:</label>
          ...
          <div className="space-y-1 text-slate-700 font-medium">
            {profile.experience !== undefined && (
              <div className="flex items-center gap-1.5">
                ...
                <span>{profile.experience} Years Professional Experience</span>
              </div>
            )}
            {profile.education && (
              <div className="flex items-center gap-1.5">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {profile.skills && profile.skills.length > 0 ? (
              profile.skills.map((sk, idx) => <SkillTag key={idx} skill={sk} />)
            ) : (
      ...
      {/* Recruiter Resume Download Link */}
      {profile.resume?.originalName && (
        <div className="pt-2 flex items-center justify-between border-t border-slate-100">
          <span className="text-xs text-slate-600 font-semibold flex items-center gap-1">
            <CheckCircle2 className="w-4 h-4 text-emerald-600" />
            Resume Attached ({profile.resume.originalName})
          </span>
          <a
            href={`/api/profile/resume/${candidateId}`}
            target="_blank"
            rel="noreferrer"
            className="px-3.5 py-1.5 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold flex items-center gap-1.5 transition-colors border border-indigo-200/80"
          >
